...
2. ~~**Structured output mode.**~~ **Done.** `--json` (single-command mode) emits a single `{"target", "exit_code", "stdout", "stderr", "error", "error_detail"}` object; multi-target mode emits a JSON array of these. See `run_command_session`/`CommandResult` in `src/launcher/rdp2exec.py`.
