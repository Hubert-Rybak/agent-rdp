...
| `src/windows/agent_rdp_bridge.cpp` | Kept the existing ConPTY-backed interactive shell path unchanged. Added `run_pipe_mode`: plain stdin/stdout/stderr pipes (no pseudo console) when `--command-file` is given, streaming stdout/stderr as separate frames plus a final exit-code frame. |
| `src/plugin/agent_rdp_client.cpp` | Ported from an `AF_UNIX` socket + `pthread` (Linux `.so`) to a Winsock TCP loopback socket + `std::thread` (Windows `.dll`). The FreeRDP/WinPR `IWTSVirtualChannelCallback`/`DVCPluginEntry` plugin API itself needed no changes — it's already cross-platform. |
| `src/launcher/agent_rdp.py` | Ported from POSIX `termios`/`SIGWINCH`/Unix sockets to Win32 console raw mode (via `ctypes`), a resize-polling thread (Windows has no `SIGWINCH`), and a TCP loopback IPC server. Removed `xdotool`/window-title-wait/Run-dialog-injection entirely; added `build_alternate_shell` to construct the AlternateShell bootstrap command line. Added binary-mode fd handling (`msvcrt.setmode`) to avoid Windows' text-mode CRLF translation and Ctrl-Z-as-EOF corrupting raw terminal bytes. |
...
- **`wfreerdp.exe`'s DVC plugin addin search path is a best guess.** `build.ps1` stages `agent-rdp-client.dll` next to `wfreerdp.exe`, matching the common "same directory as the client" convention, but this wasn't empirically confirmed against a real FreeRDP Windows build.
...
2. ~~**Structured output mode.**~~ **Done.** `--json` (single-command mode) emits a single `{"target", "exit_code", "stdout", "stderr", "error", "error_detail"}` object; multi-target mode emits a JSON array of these. See `run_command_session`/`CommandResult` in `src/launcher/agent_rdp.py`.
3. **MCP server wrapper.** Wrap `agent_rdp.py` as an MCP tool so an agent calls it directly (structured args/results) instead of shelling out to a subprocess and parsing stdout/stderr/exit code.
4. **Credential handling (implemented, unverified on Windows).** `--credential-target` reads the RDP password from Windows Credential Manager and `--save-credential` stores it (`read_windows_credential`/`write_windows_credential`, `ctypes` against `advapi32`). Resolution order is `-P` → `RDP_PASSWORD` → Credential Manager → interactive prompt. The `ctypes`/`advapi32` path is import-guarded and unit-tested for resolution order, but the live `CredReadW`/`CredWriteW` round-trip is unverified without a Windows host.
5. **CI.** ~~There is no automated build/test today~~ — `.github/workflows/ci.yml` now runs the launcher's `py_compile` + `pytest` suite on Ubuntu and builds the native components on `windows-latest` on every push. The remaining gap is a *live* RDP smoke test (item 1), which still needs a real target.
6. **Concurrency (implemented, unverified on Windows).** A comma-separated target list (`user@h1,user@h2`) or `--targets-file` fans one command out across targets in parallel via a `ThreadPoolExecutor` bounded by `--max-parallel`; each target gets its own ephemeral temp dir + OS-assigned loopback port + `wfreerdp` process (`run_multi_target`). Results aggregate into a JSON array with an all-or-nothing exit-code rollup. The aggregation/exit-code logic is unit-tested; the concurrent live RDP legs are unverified without real targets.
7. ~~**Error surface for agents.**~~ **Done.** `classify_error` folds the bridge's `kError` text into a small closed slug set (`conpty_unavailable`, `pipe_setup_failed`, `process_setup_failed`, `process_spawn_failed`, `remote_error`) plus launcher-side conditions (`connect_timeout`, `channel_dropped`, `local_setup_error`, `auth_required`), surfaced as the `error` field with the raw text preserved in `error_detail`. See the vocabulary table in `README.md`.

### Verification status of the follow-up work

The #2/#7 output and error vocabulary, the #6 aggregation/exit-code rollup, and the #4 resolution order are covered by `pytest` (`tests/`), runnable on any OS and wired into CI. The Windows-only pieces (#4's `advapi32` calls, #6's concurrent live RDP sessions) are implemented and statically/structurally checked but, like the rest of this port, remain unverified against a real Windows RDP target.
